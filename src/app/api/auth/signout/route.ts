 ...
 * the adapter, then 303s home. Cookie deletion is immediate for this browser;
 * an already-issued access token verified locally elsewhere stays valid until
...
export async function POST(req: Request) {
  ...
  const response = NextResponse.redirect(new URL('/', selfOrigin), 303);
