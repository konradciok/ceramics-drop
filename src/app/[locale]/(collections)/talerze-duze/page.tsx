...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  assertCategoryPublic('talerze-duze');
  const t = await getTranslations({ locale });
  return {
    title: t('title.talerzeDuze'),
    description: t('meta.collections.talerzeDuze'),
    alternates: alternatesFor(locale as Locale, '/talerze-duze'),
  };
}
...
export default async function Page({ params }: Props) {
  ...
  return (
    <main>
      <JsonLd data={collectionSchema({ slug: 'talerze-duze', locale: locale as Locale, t, soldIds })} />
      <CollectionScreen slug="talerze-duze" />
    </main>
  );
}
