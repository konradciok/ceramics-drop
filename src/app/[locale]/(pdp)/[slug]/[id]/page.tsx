...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (slug === PRINTS_SLUG) {
    ...
    const displayName = `${t('product.print')} Nº ${design.num}`;
    const rawNotes = t.raw(`notes.${PRINTS_SLUG}`) as unknown;
...
export default async function Page({ params }: Props) {
  ...
  // Fine-art print PDP — separate registry, no piece_state / sold overlay.
  if (slug === PRINTS_SLUG) {
    const design = getPrintById(id);
    if (!design || !design.published) notFound();
    const t = await getTranslations({ locale });
