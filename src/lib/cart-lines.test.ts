...
describe('resolveCartLines', () => {
  it('resolves a mixed cart preserving order', () => {
    const lines = resolveCartLines(['k01', 'print:fap005:50x70:true:false:black']);
  ...
  it('keeps two distinct variants of the same design as separate lines', () => {
    const lines = resolveCartLines(['print:fap005:30x40:false:false:none', 'print:fap005:50x70:false:false:none']);
    expect(lines).toHaveLength(2);
  });

  it('dedupes identical entries', () => {
    expect(resolveCartLines(['k01', 'k01'])).toHaveLength(1);
    expect(resolveCartLines(['print:fap005:50x70:true:false:black', 'print:fap005:50x70:true:false:black'])).toHaveLength(1);
  });

  it('drops unknown ids, malformed tokens, and unavailable/unpublished prints', () => {
    expect(resolveCartLines(['nope'])).toHaveLength(0);
    expect(resolveCartLines(['print:fap005:50x70:false'])).toHaveLength(0); // malformed (too few parts)
