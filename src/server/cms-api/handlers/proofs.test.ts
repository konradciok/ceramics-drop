...
const ASSET_ID = '92000000-0000-0000-0000-000000000001';

describe('proofDecisionRoute', () => {
  it('rejects a non-UUID proofId before touching the database', async () => {
    const rpc = vi.fn();
    const res = await proofDecisionRoute.handler(req({ expectedRevision: 1, action: 'approve' }), {} as CloudflareEnv, { id: 'fap001', proofId: 'not-a-uuid' }, ctxWith(rpc));
    expect(res.status).toBe(422);
    expect(rpc).not.toHaveBeenCalled();
  });

  it('requires expectedRevision and a valid action', async () => {
    const res = await proofDecisionRoute.handler(req({ action: 'nope' }), {} as CloudflareEnv, { id: 'fap001', proofId: ASSET_ID }, ctxWith(vi.fn()));
    expect(res.status).toBe(422);
  });
  ...
  it('maps invalid_proof_transition to 409', async () => {
    ...
    const res = await proofDecisionRoute.handler(req({ expectedRevision: 1, action: 'reject' }), {} as CloudflareEnv, { id: 'fap001', proofId: ASSET_ID }, ctxWith(rpc));
    expect(res.status).toBe(409);
    expect((await res.json()).code).toBe('INVALID_PROOF_TRANSITION');
  });

  it('succeeds and returns the reloaded product, passing p_product_id from the path', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('decide_print_proof', {
      p_asset_id: ASSET_ID,
      p_product_id: 'fap001',
      p_action: 'approve',
