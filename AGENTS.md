...
Cloudflare Workers via OpenNext (`open-next.config.ts`). Preview locally with `npm run preview:cf`. Workers Builds CI handles production deploys on push to main. The configured incremental-cache adapter reads prerendered/static assets and deliberately does not persist writes; the OpenNext tag-cache adapter is `dummy`. Generated cache Durable Object exports are still required by the OpenNext worker bundle, but they do not make application data-tag invalidation available. Catalog and print-pricing DB reads therefore bypass Next's persistent data cache, and catalog-consuming routes must remain absent from the generated prerender manifest; `npm run build` checks that contract after every Next build.
