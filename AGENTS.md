...
|---|---|---|
| `npm run orders` | Order/inventory inspection CLI + the four admin mutations (refund, release-reservation, resend-confirmation, create-shipment) + `webhook-config-check` (live Stripe endpoint vs `HANDLED_STRIPE_EVENTS` drift guard) + `reconcile-refunds` (sweep fully-refunded charges the DB hasn't converged — dry-run by default; `--confirm <uuid>` converges one order, `--skip-relist` converges pieces to the private-sale terminal off-sale state). Mutations need `--confirm <order-id>`; non-prod blocked without `--allow-nonprod`; PII redacted by default | `docs/orders-cli.md` |
...
| `npm run print-assets:editorial` | Storefront editorial/lifestyle gallery WebPs (3 slides/design) from `design/uploads/master-images-prints/print-{NNN}/`, mirrored to `public/uploads/` only (no R2); `--product <id>` or `--all` (validates every design's source folder first, incl. drafts), optional `--dry-run`; a successful non-dry-run `--all` also writes paste-ready `editorialGallery` registry snippets | `docs/print-asset-runbook.md` |
...
Ceramic products are defined statically in `src/lib/products.ts`. The registry is built in two passes: `buildBase()` generates pieces with stable ids, then `buildProducts()` applies the inventory-review diff (`REMOVED` / `RECATEGORISE` / `APPEND_ORDER` / `GALLERY_MERGE`) and assigns display `num` + `noteIndex`. The public accessors (`getProducts`, `getProductsByCategory`, `getProductById`, `getPublicProducts`, `resolveCartProducts`) are **async** and read a DB-driven catalog at runtime behind the `CATALOG_SOURCE` env: **production sets `db`** (`wrangler.jsonc`), so they read the Supabase catalog shadow tables in `src/lib/catalog/`; `code` (the unset default) reads `products.ts` and is the local/test fallback. DB-mode catalog and print-pricing loaders read Supabase directly on each invocation: the deployed OpenNext configuration has no persistent tag cache, so these reads must not be wrapped in `unstable_cache` or coupled to a cache-invalidation endpoint. Every public catalog consumer is server-rendered on demand; in particular, the home page, fine-art-print collection, and sitemap explicitly use `force-dynamic` because they would otherwise be indefinitely prerendered from code-mode build data. `npm run build` fails closed against the generated Next manifests in `postbuild`; `npm run build:opennext` separately checks the final copied OpenNext manifests and build ID before preview/deploy. `HIDDEN_CATEGORIES` (currently empty) is the mechanism to withdraw an entire family from the storefront without deleting it. The database (`piece_state` table) is the source of truth for sold/reserved state; under `CATALOG_SOURCE=db` the catalog shadow tables (`products` / `product_variants` / `product_media`) are what the storefront reads, seeded from the code registry — which remains the structural source of truth and provides the sync helpers used by client/admin surfaces.

`npm run catalog:backfill` is a structural sync, not a print-publication operation. It sends the complete registry projection through the transaction-scoped `backfill_catalog()` RPC: products, replaced variants, and replaced media either all commit or all roll back. A newly seeded print whose registry target is `active` is staged as `draft`; an existing print keeps its current database status, while registry-archived rows stay archived. Any preserved active print is rechecked after variant replacement before commit. Fine-art-print activation is enforced in the database: `products.status` defaults to `draft`, direct active inserts/updates are guarded by a trigger, and admin activation goes through `update_product_status_guarded()`. Both guards and the backfill share the same readiness predicate: at least one active variant must exist, and every active variant must have a matching, ready, non-revoked asset assignment.
...
**CMS content layer** (`src/app/admin/content` + `/api/admin/content/*` + `/api/admin/products/[id]`) drafts/previews/publishes product notes and status against the `cms_documents` table; publish is atomic via the `publish_cms_version()` RPC (migration `20260709120000`), notes keyed by product id. This is the editorial surface — with `CATALOG_SOURCE=db` live in production, publishes write the status/notes the storefront reads from the DB; the code registry remains the structural source of truth that seeds those rows. The same table also carries two other fixed-shape documents through the same admin surface: `page:print-pdp` (per-locale draft/preview/publish, edited via a dedicated `PrintPdpEditor`) drives the three info accordions and About-the-Artist band on fine-art-print PDPs; `page:home` (edited via `HomeHeroEditor.tsx`) drives the homepage hero (see Storefront Surfaces below). Both fall back to committed copy/images when nothing is published, so the storefront renders correctly before any document is published. `page:home`'s hero media (images/video) is uploaded via `/api/admin/content/media` (`src/lib/admin/site-media-upload.ts`) as content-addressed objects (`<sha256>.<ext>`) under a `site-media/` prefix in the existing `PRINT_ASSETS` R2 bucket, and served publicly via `/api/media/[key]` (`src/lib/site-media.ts` holds the key-format validation regexes).
...
- **Homepage** (`/`) — CMS-managed full-bleed hero (`page:home` document, admin editor `HomeHeroEditor.tsx`; see CMS content layer above) with a fallback to committed static images/copy when unpublished, plus a daily-rotated fine-art-print rail (`src/lib/print-rotation.ts`, seeded shuffle so the same tiles show all day). `force-dynamic` like other public catalog consumers.
- **Collection pages** — one per ceramic category (`/{category}`), grouped grids of tiles.
- **`/sklep` hub** — every piece grouped by category in a single `GroupedGallery` with a sticky category jump-nav (scroll-spy); the nav Shop link points here.
...
- **`/showroom`** — pieces retired from a past drop into showroom state (visible, not purchasable — see `piece_state.showroom` above); visitors can register interest via `/api/interest` (see Other API Routes).
...
`src/lib/webhook.ts` → `handleStripeEvent()` handles Stripe webhooks at `/api/stripe/webhook` (snapshot / full-payload event destination). The route wraps every delivery in the **shared `webhook_events` idempotency ledger** (also used by the Prodigi callbacks): a leased CAS claims the event, and any handler throw releases the lease so Stripe's retry reclaims it immediately instead of being deduped-and-dropped inside the lease window. A **200 dedupe** is reserved for a ledger row that reached `done` — any competing delivery (active lease, lost claim CAS, 23505 insert race over a non-done row) answers **409** so Stripe redelivers after the lease (Plan 06; expect concurrent duplicate deliveries to show as failed-then-succeeded in Stripe — do not tune the 409s away).

**API-version ritual:** stripe-node ≥ v12 always pins its **bundled** API version on every request — the account-default version is never used (https://docs.stripe.com/sdks/set-version). `src/lib/stripe.ts` additionally passes that version explicitly (`STRIPE_API_VERSION`, currently `2026-05-27.dahlia` from `stripe@22.2.1`); the `apiVersion` option is typed as the bundled-version literal, so an `npm update stripe` that moves the bundled version fails `npm run typecheck` instead of silently changing payload shapes. The ritual on every `stripe` package bump: update `STRIPE_API_VERSION` in `src/lib/stripe.ts`, update the snapshot webhook endpoint's API version (Stripe Dashboard) in lockstep so incoming event payloads stay aligned with the SDK types, then verify with `npm run orders -- webhook-config-check`. (Confirm the exact bundled version from the installed `stripe` package / `package-lock.json` after a bump — it is not inferable from the `^` range in `package.json`.)

Event handling (the full set is `HANDLED_STRIPE_EVENTS` in `src/lib/webhook.ts`; the live endpoint's subscriptions are drift-gated by `npm run orders -- webhook-config-check`):
...
- `charge.dispute.created` → alert-only (Sentry + a due-date-bearing studio email — the evidence clock starts at creation and a missed deadline is an automatic loss); the send is allowed to throw so Stripe redelivers a failed alert; no order state changes until the dispute closes
- `refund.failed` → alert-only (Sentry + studio email); the order stays `refunded` in the DB — a refund the customer never received (closed account / expired card, up to ~30 days later) has no automatic remedy and a human must re-issue it by another way

`worker.ts` wraps the OpenNext handler with two extra handlers: a **cron** (every 15 min) that runs five sweeps — (1) expire abandoned orders older than 1 hour (claims a recoverable `expiry_claim_at` lease BEFORE the irreversible PaymentIntent cancel, then frees reserved pieces — pieces first, lease-fenced `pending→expired` CAS last; a paid-on-pending `refund_pending_at` marker stranded >24 h fires the M-15 alert), (2) alert on `failed_action_required` fulfilment jobs, (3) the M-10 watchdog for jobs stranded >2 h in a pre-submission state (queued / submitting / retryable), (4) the M-12 reconciliation sweep that re-polls stale post-submission Prodigi orders and re-runs the callback merge, and (5) the promo-redemption reconciliation sweep (`src/lib/promo-reconcile.ts`) that converges pending `promo_redemptions` left stale >2 h and alerts if a paid order still has one (`markPaid` should have settled it) — every sweep self-alerts to Sentry (plus logs) if it throws, since a dead `scheduled` handler would otherwise show green in Cron Past Events; and a **queue consumer** that drains the `prodigi-fulfilment` queue (`src/server/fulfilment/process-job.ts`; transient errors retry with exponential backoff capped at 1 h) and an **alert-only `prodigi-fulfilment-dlq` consumer** (per batch: log + Sentry + one studio email, then ack every message — it never retries, so a poison message cannot loop back through the system).
...
- **Curation:** `src/lib/print-collections.ts` + `print-curation.ts` group designs (by id) into named, ordered `PRINT_COLLECTIONS` for display on `/fine-art-prints`, with an `UNASSIGNED_COLLECTION` ("inne") fallback bucket for anything not listed. Membership is kept out of `PrintDesign` itself — under `CATALOG_SOURCE=db` the catalog mapper rebuilds designs field-by-field from DB rows, so a registry-only field would be silently dropped.
...
A back-office UI lives **outside** the `[locale]` tree at `src/app/admin/` with its own `<html>`/`<body>` (overview KPIs, `orders`, `fulfillment`, `customers`, `inventory`, `products`, `content` — the last two drive the CMS publish path described under Product Registry; `promotions` manages promo codes with live redemption/revenue stats, on the same server-page → client-editor → `/api/admin/*` pattern as `pricing` — see `docs/promo-codes.md`). Server helpers are in `src/lib/admin/` (`data.ts` reads — `listOrders`/`getOrder`/`listInventory` take an optional injected Supabase client, defaulting to `adminSupabase()`; `clients.ts` client factories — `adminSupabase()`/`adminStripe()` build from `getCloudflareContext()`, plus pure `adminSupabaseFromEnv()`/`adminStripeFromEnv()` for callers outside a Workers request; `actions.ts` the four dependency-injected mutations; `fulfillment.ts` queue stages; `money.ts`, `route-helpers.ts`). Mutating actions are exposed under **`/api/admin/*`**: `refund`, `create-shipment`, `label`, `release-reservation`, `resend-confirmation` — each route is a thin adapter over the matching `src/lib/admin/actions.ts` function. `set-piece-status` (mark a piece sold/available from the inventory screen, `InventoryManager.tsx`), `toggle-showroom` (move pieces into/out of showroom), and `end-drop` (bookkeeping-only: `drops.status → ended`) all talk to Supabase directly rather than through `src/lib/admin/actions.ts` and have no Orders CLI equivalent.
...
- **`/api/interest`** — anonymous, rate-limited (10/min/IP) showroom-interest capture; writes to `product_interest`, always answers `200 { ok: true }` regardless of new/duplicate so submission history is never leaked.
- **`/api/feed/google`** + **`/api/feed/meta`** — Google Shopping & Meta Catalog product feeds, **fine-art prints only**: one row per published design per locale, `g:id` = design id (`fapNNN`), always `in stock` (print-on-demand). `FEED_LOCALES` in `src/lib/feed.ts` covers all 4 locales; PLN for `pl`, EUR for every other locale — GBP is never fed. **Ceramics are deliberately excluded.** Each piece is one-of-a-kind, so a piece sold minutes after a fetch stays advertised until the platform's next scheduled fetch (daily at best) — an availability mismatch by construction, not a tunable bug. Consequence, decided and accepted (owner, 2026-09-21): ceramic `content_ids`/`item_id` values still emitted by the pixel + CAPI (`src/lib/analytics.ts`) no longer resolve to a catalogue row. The studio runs no Meta catalogue / dynamic product ads for ceramics, which are the only surfaces needing a catalogue match; ceramic events still carry `value` + `currency`, so conversion reporting is unaffected. Reopen only if catalogue-backed ceramic ads are ever wanted — that reopens the feed decision, not the analytics one.
...
- **`/api/promo/validate`** — rate-limited, read-only preview of a promo code's discount for the current cart + currency (no DB writes, no reservation, no claim); `/api/checkout` re-validates authoritatively. See `docs/promo-codes.md`.
...
- **`/api/media/[key]`** — public GET/HEAD for CMS-managed site-media (homepage hero images/video); keys are content-addressed (`<sha256>.<ext>`), validated against `src/lib/site-media.ts`'s regexes, served from the `site-media/` prefix in the `PRINT_ASSETS` R2 bucket.
- **`/api/admin/content/media`** — admin-gated site-media upload backing the `page:home` CMS document (see CMS content layer above).
...
- `piece_state`: `product_id` PK, `status` (available|reserved|sold), `reserved_until`, `order_id`, `showroom` (bool — visible but not purchasable, independent of `status`) + `showroom_entered_at` + `showroom_note`
- `drops`: `id`, `status` (active|ended), `ended_at` — ended via `/api/admin/end-drop`; ending a drop is pure bookkeeping and does NOT auto-move its pieces into the showroom (that's a separate, explicit `toggle-showroom` step)
- `product_interest`: anonymous showroom-interest capture (`product_id`, `email`, `message`, `consent_marketing`, `locale`), deduped per (`product_id`, `email`) via `POST /api/interest`; notifies the studio by email on genuinely new submissions only
- `orders`: UUID id, `payment_intent_id`, `status` (pending|paid|failed|expired|refunded), totals in minor units, contact JSON, `delivery_method`, `locale`, `currency` (pln|eur|gbp), `marketing` (jsonb — server-conversion context), `confirmation_email_sent_at`, `conversions_sent_at` (claim column — server-side purchase conversions are sent at most once per order), `private_sale_id` (nullable FK), `user_id` (nullable FK → `auth.users`, `ON DELETE SET NULL` — customer-account link, set at checkout or by verified-email backfill-on-login) + `user_unlinked_at` (deletion stamp; permanently excludes rows from backfill), `resend_email_id` (links the order to its Resend confirmation email so the Resend webhook can Sentry-alert on a bounce/complaint), `promo_code` (nullable, denormalized) + `discount` (minor units, default 0 — `subtotal` stays pre-discount; `total = subtotal − discount + shipping`)
...
- `promo_codes` + `promo_redemptions`: promo-code definitions (percent or per-currency fixed amount, `applies_to` track, schedule window, `max_redemptions`, `newsletter_welcome` — at most one active) and their redemption ledger (`pending`→`redeemed`/`released`), claimed/settled by the `claim_promo_redemption()` / `settle_promo_redemption()` RPCs (migration `20260830120000_promo_codes.sql`). See `docs/promo-codes.md`.
...
- `NEXT_PUBLIC_GOOGLE_PLACES_API_KEY` — address autocomplete on the print checkout delivery form (`AddressAutocomplete.tsx`)
...
**API error responses:** `NextResponse.json({ error: reason }, { status: code })`. Checkout returns 400 (validation — print carts fail delivery validation with `{ error: 'invalid_delivery' | 'invalid_contact' | 'invalid_address' }` from `validatePrintDelivery`), including `{ error: 'private_sale_prints_unsupported' }` when a private-sale token is sent with a print cart, `{ error: 'invalid_promo', reason }` for an ineligible promo code (`reason` one of `not_found|inactive|not_started|expired|wrong_track`), 409 with `{ error: 'unavailable', sold: string[] }` (pieces unavailable), 409 `{ error: 'order_conflict' }` (stale attemptId — client must reset it), 409 `{ error: 'checkout_in_progress' }` (a concurrent or unresolvable POST for the same attemptId — client must KEEP the attemptId and retry), 409 `{ error: 'print_asset_unavailable' }` (no ready/usable print asset for the requested variant — runs before reservation/PI), 409 `{ error: 'promo_exhausted' }` (promo capacity gone, including a race lost after PI creation — full rollback), 502 with `{ error: 'stripe_failed' }` (Stripe PI creation failure), 503 `{ error: 'print_asset_error' }` (transient Supabase failure resolving the print asset — safe to retry, before any reservation/PI), 500 (other server faults, including `{ error: 'promo_claim_failed' }` when the redemption-claim RPC itself errors).
...
Cloudflare Workers via OpenNext (`open-next.config.ts`). Preview locally with `npm run preview:cf`. Workers Builds CI handles production deploys on push to main. The configured incremental-cache adapter reads prerendered/static assets and deliberately does not persist writes; the OpenNext tag-cache adapter is `dummy`. Generated cache Durable Object exports are still required by the OpenNext worker bundle, but they do not make application data-tag invalidation available. Catalog and print-pricing DB reads therefore bypass Next's persistent data cache, and catalog-consuming routes must remain absent from generated prerender manifests. Next `postbuild` and the post-OpenNext copy guard both fail closed when required artifacts are missing; preview, deploy, and Wrangler's custom build all use the guarded OpenNext build.

Key `wrangler.jsonc` bindings: `ASSETS` (static assets from `.open-next/assets`), `WORKER_SELF_REFERENCE` service binding (self-reference for cache purging), `FULFILMENT_QUEUE` (Cloudflare Queue `prodigi-fulfilment`, consumer with exponential retry/backoff plus a dead-letter queue `prodigi-fulfilment-dlq` consumed alert-only), `PRINT_ASSETS` (R2 bucket for high-res print files), and a cron trigger every 15 min.
