...
Visibility is layered: `isProductPublic` checks `status` (`active|draft|hidden|archived`; undefined ⇒ `active` in code mode) **and** the hidden-family set; `isProductPurchasable` additionally excludes `sold` and `showroom` pieces (both still render, with a badge). Each piece carries a `dropId` (default `drop-1`, overridable via `DROP_OVERRIDE`); drops are rows in the `drops` table (active/ended + label — minted with `npm run drop:create`, ended from `/admin/inventory` via `/api/admin/end-drop`, which deliberately moves no pieces). Retiring pieces into the **showroom** is the separate explicit step (`/api/admin/toggle-showroom` → `piece_state.showroom`): showroom pieces stay visible but `reserve_pieces()` rejects them, and visitors can register interest via `/api/interest`.

**CMS content layer** (`src/app/admin/content` + `/api/admin/content/*` + `/api/admin/products/[id]`) drafts/previews/publishes product notes and status against the `cms_documents` table; publish is atomic via the `publish_cms_version()` RPC (migration `20260709120000`), notes keyed by product id. Product **status** changes go through the `update_product_status_guarded()` RPC (migrations `20260717*`): an atomic transition that, when activating a print, verifies full ready print-asset coverage under the same product lock `publish_print_asset_revision()` takes — no TOCTOU with a concurrent publish or revoke. This is the editorial surface — with `CATALOG_SOURCE=db` live in production, publishes write the status/notes the storefront reads from the DB; the code registry remains the structural source of truth that seeds those rows.
...
- **`/showroom`** — retired showroom pieces (also under `(collections)`): still visible, not purchasable, each with a "register interest" form posting to `/api/interest`.
...
- **`/api/interest`** — showroom interest capture (email + optional message + marketing consent) upserted into `product_interest` (unique per piece+email); showroom pieces only, per-IP rate-limited, always answers `200 { ok: true }` so submission history never leaks; genuinely new submissions email the studio.
...
- `print_fulfilment_assets` + `print_variant_asset_assignments`: immutable, content-addressed print files in R2 (status `staged|ready|retired|revoked`; content columns frozen by trigger once past `staged`) and the live asset per (product, variant); `publish_print_asset_revision()` atomically swaps a whole revision in after validating variant coverage + dimensions against `product_variants.print_area_*_px` (migration `20260711120000`)
- `drops` + `product_interest`: drop lifecycle (active|ended) and showroom interest submissions (unique per piece+email); the same migration (`20260709130000`) added the `piece_state.showroom*` columns and made `reserve_pieces()` reject showroom pieces
- `products` / `product_variants` / `product_media`: catalog shadow tables the storefront reads under `CATALOG_SOURCE=db`, seeded/backfilled by `npm run catalog:backfill`; `cms_documents` backs the CMS layer; `catalog_audit_log` records every admin product mutation and print-asset publish (before/after + actor)
- `webhook_events`: provider-webhook dedup store (unique per provider + event id; used by the Prodigi callback handler)
...
`src/middleware.ts` must **not** be renamed to `proxy.ts`: OpenNext only bundles edge-runtime middleware, but Next 16's `proxy.ts` is Node-runtime only and OpenNext rejects it, breaking the Cloudflare build (`next build` alone does not catch this).

New migrations go in `supabase/migrations/` with timestamp prefix. Docs for deployment, E2E testing design, and analytics setup are in `docs/`; operational runbooks: `docs/stripe-operations.md`, `docs/print-asset-runbook.md`, `docs/orders-cli.md`, `docs/prodigi-cli.md`. Design specs and implementation plans (including in-flight work) live under `docs/superpowers/` (`specs/`, `plans/`) and `docs/plans/`; benchmark/competitor research under `docs/research/`.
