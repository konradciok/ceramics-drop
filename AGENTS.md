...
Operational / one-off scripts (full list in `package.json`): `npm run prodigi` (Prodigi API v4 CLI — sandbox quotes/orders, SKU lookup, order inspection; see `docs/prodigi-cli.md`), `npm run prodigi:contract-smoke` (manual sandbox create→inspect→cancel contract smoke proving the real mapper+client round-trip — audit H-1; runs locally or via the `prodigi-contract-smoke` workflow_dispatch; see `docs/prodigi-contract-smoke.md`), `npm run orders` (order/inventory inspection CLI + the four admin mutations — refund, release-reservation, resend-confirmation, create-shipment; see `docs/orders-cli.md`), `npm run private-sale:create` (mint a re-sale link for sold pieces), `npm run apple:client-secret` (mint the Sign in with Apple ES256 client-secret JWT for the Supabase provider — 6-month max validity, rotation procedure in `docs/customer-accounts-runbook.md`), `npm run sync-prodigi-skus` (verify/upsert Prodigi print SKUs into `pod_variants`), `npm run print-assets:onboard` (batch-onboard new print designs from a manifest — `config/print-assets/onboarding-manifest.json`, see `.example.json` — plus an intake folder `design/print-assets/_incoming/`; validates each master's resolution against every variant it will offer via `assetPxFor()`, copies the master + shared signature into place, writes each `config/print-assets/{id}.json`, and emits ready-to-paste `PrintDesign` entries to `design/print-assets/_incoming/generated-prints-entries.ts` — never edits `prints.ts` or the DB itself; `--dry-run`/`--force`/`--manifest <path>`; see `docs/print-asset-runbook.md` § Batch onboarding), `npm run print-assets:prepare` / `print-assets:upload` / `print-assets:verify` / `print-assets:publish` (the print-fulfilment asset pipeline — compose exact Prodigi print-area derivatives from an artwork master + background + SVG signature using a proportional layout, upload to R2 + stage rows, verify byte-identity + promote to `ready`, then atomically assign a revision to every active variant; all take `--product`/`--revision`, publish requires `--confirm <revision>`, all support `--dry-run`; need wrangler auth + `SUPABASE_*` (`upload`'s conditional create-only R2 PUT additionally needs the CLI-only `R2_S3_ACCOUNT_ID`/`R2_S3_ACCESS_KEY_ID`/`R2_S3_SECRET_ACCESS_KEY`, scoped to the print-assets bucket only and resolved lazily before its first real create — never set on the Worker), and honour a `PRINT_ASSETS_BUCKET` override from `.dev.vars`/`--env-file`; manifests are schema-v2-only for `upload`/`verify`, `publish` alone also accepts a validated legacy projection for rollback; see `docs/plans/print-asset-pipeline.md` and operator procedures in `docs/print-asset-runbook.md`), `npm run print-assets:gallery` (storefront gallery WebPs from a published fulfilment derivative — uploads to R2 under `prints/{productId}/gallery/{slot}/` and mirrors to `public/uploads/`; `--product` required, optional `--slot`/`--revision`/`--dry-run`), `npm run print-assets:mockups` (pre-rendered configurator hero mockups — composes published FAP/CFPM derivatives into shared frame masters from `config/print-assets/frames.json`; `--product` required, optional `--state`/`--revision`/`--dry-run`; ships together with the design's `mockups: true` flag), `npm run print-assets:sandbox-matrix` (one Prodigi sandbox order per distinct print-area profile using production signed asset URLs; `--product`, optional `--run-id`/`--dry-run`), `npm run print-asset:smoke` (HEAD probe of a signed print-asset URL against a live origin — redacts `sig` in output; requires `PRINT_ASSET_TOKEN_SECRET` + `SUPABASE_*` + a `ready`/`retired` asset row), `npm run print-fulfilment:check-jobs` / `print-assets:inventory` (Phase 0/6 pre-cutover gates), `npm run reconcile:orders` (backfill missed emails / stuck InPost shipments for prod orders — dry-run by default), `npm run notes:generate` (draft product notes), `npm run i18n:pull` / `i18n:push` / `i18n:check` / `i18n:organize` (sync UI strings with Notion — see `docs/notion-i18n.md`), `npm run gtm:setup` / `gtm:list` / `gtm:key` (Google Tag Manager API), `npm run ga4:report` / `bq:query` (GA4 Data API + BigQuery export reports), `npm run generate-image-variants` (variant-only image optimise), `npm run cf-typegen` (regenerate Cloudflare env types), `npm run cf:dns-cleanup` / `cf:com-redirect` (Cloudflare DNS + `.com→.studio` redirect maintenance), `npm run admin:dev:lan` + `admin:proxy` (run the `/admin` UI over LAN via an nginx reverse proxy — see Admin below).
...
- **`/konto`** — customer accounts via **server-only Supabase Auth** (sessions are httpOnly cookies, verified locally with jose against the project JWKS — no browser Supabase client, no `NEXT_PUBLIC_*` key). **Google sign-in is live and verified end-to-end in production (2026-07-25).** Apple sign-in is wired in code but not yet enabled — blocked on an Apple Developer Program membership decision (`docs/customer-accounts-runbook.md` §1.3/§1.4). Order history plus per-order detail/tracking at `/konto/zamowienia/[id]` (ownership-filtered, 404 on mismatch). Both pages `force-dynamic` + noindex; the header `Konto` link is deliberately **static** (session-aware rendering there would flip the prerenderable `pl` tree dynamic). Fail-closed behind the `SUPABASE_PUBLISHABLE_KEY` secret: unset ⇒ pages render "accounts unavailable" and `/api/auth/*` 404 (currently **set** — the feature is live). Plan: `docs/plans/customer-accounts.md`; operations (provider setup, Apple secret rotation, deletion runbook): `docs/customer-accounts-runbook.md`.
