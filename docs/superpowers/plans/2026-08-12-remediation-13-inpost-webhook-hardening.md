...
- [ ] **Make the advance atomic — a read-then-unconditional-write leaves a race** (an older event reads the old status, a newer event advances it, then the older event writes last and regresses the row). Do **not** read the rank in JS and then issue an unconditional UPDATE. Instead gate the write with a conditional predicate evaluated **in the database** so only a strictly-higher rank wins:
  - **Preferred:** add a `delivery_status_rank smallint` column (tiny additive migration), written alongside `delivery_status`. **Backfill existing rows in the same migration** — `update orders set delivery_status_rank = <rank_of(delivery_status)> where delivery_status_rank is null` using a CASE mapping the known statuses — otherwise every existing order has `rank IS NULL` and the first incoming event (even a delayed *lower*-ranked one) would qualify and could overwrite a terminal `delivered`/`returned_to_sender`. The update predicate is then simply `where id=$id and delivery_status_rank < $newrank` (no `IS NULL` escape hatch once backfilled). For a legacy row whose stored `delivery_status` isn't in the known vocabulary, backfill to rank `0` (so any known incoming status can advance it, but the write is still rank-gated) and log it. Atomic, monotonic, no lost-update.
  - **No-migration alternative:** put the current-status→rank mapping in the `WHERE` clause as an inline `CASE` expression that derives the stored row's rank from its `delivery_status` and compares to `$newrank` — this needs no backfill (rank is computed from the existing status each time), so it sidesteps the NULL-rank problem entirely, at the cost of a verbose predicate. An unknown legacy `delivery_status` maps to rank `0` in the CASE. (Optimistic `where delivery_status = <value-we-read>` + retry is a weaker fallback; prefer one of the two above.)
...
- [ ] Failing test additions: two concurrent-ish events applied out of order (higher rank first, then lower) → the row holds the higher status; the lower-rank write affects **zero rows** (assert the predicate). Plus a **legacy-row** case: an existing terminal row (`delivered`, backfilled/derived to top rank) receives a delayed lower-ranked event → **not** overwritten (this is the exact NULL-rank regression the backfill/derive prevents).
...
**Optional (Task 2 atomicity, preferred path):** one tiny additive column `alter table orders add column if not exists delivery_status_rank smallint;` **plus a backfill in the same migration** — `update orders set delivery_status_rank = <CASE rank_of(delivery_status)> where delivery_status_rank is null` (unknown legacy statuses → `0`). The backfill is **required**, not optional: without it every existing order has `rank IS NULL` and a delayed lower-ranked event could regress a terminal status. Backward-compatible (old code ignores the column); auto-applies on merge. The no-migration inline-`CASE` alternative in Task 2 derives rank from `delivery_status` on each write and needs no column/backfill — pick one.
