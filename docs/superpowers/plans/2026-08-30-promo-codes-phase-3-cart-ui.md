...
**Interfaces:**
- Consumes: Phase 2's checkout contract (`promo_code` in POST body, `invalid_promo`/`promo_exhausted` errors, `discount` in the success JSON — present **only** when a promo was applied; a no-promo response has no such field); Phase 1 domain functions.
...
- [ ] **Step 2: Wire checkout** in `handleCheckout()` (~346): include `promo_code` in the POST body when applied. Handle new error responses: 400 `invalid_promo` / 409 `promo_exhausted` → clear the applied code, show the message, **do not** clear the rest of the checkout form (match how `unavailable`/409 conflicts are surfaced today). **Critical (hard requirement, mirrored in the acceptance checklist):** the applied code participates in the attempt identity — wherever cart-content changes currently regenerate `attemptId` (`acc_checkout_attempt_v1`), applying/removing/changing a code must ALSO regenerate it. Two server-side contracts depend on this: (a) Phase 2's Stripe idempotency key `pi_create_${orderId}` is amount-sensitive (same key, different amount → Stripe 400), and (b) `claim_promo_redemption` returns `false` for an order id that already holds a live claim for a *different* promo (one code per order) — a stale attemptId carrying a changed code would surface as a misleading 409 `promo_exhausted`. Find the existing regeneration trigger and extend it; add a unit test in `src/lib/checkout-client.test.ts` if the attempt logic lives there.
...
- [ ] **HARD GATE — do not sign off the phase without it:** applying, removing, or changing a code regenerates `attemptId`, proven by a unit test. This guards the `claim_promo_redemption` re-entrancy edge (a reused order id with a different promo is rejected by the RPC) and the amount-sensitive Stripe idempotency key — see Task 2 Step 2.
