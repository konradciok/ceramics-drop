...
- [ ] Pin jose algorithms: in the customer-session verify and the Access JWT verify, pass `algorithms: [<observed alg>]` — read the actual `alg` from each JWKS during implementation (Supabase: ES256 for new signing keys / RS256 legacy — pin to what the project's JWKS serves; CF Access: RS256). Tests: a token signed with a different alg is rejected (if cheaply mockable; otherwise assert the option is passed).
...
- [ ] L-30: rewrite the `clients.ts:1-9` header comment to the truth (tracked file; `ADMIN_*` overrides are a local-dev convenience); add a runtime guard: if any `ADMIN_*` override is set while the request host is production, log a structured warning (`admin_env_override_in_prod`) — do not throw (an operator may have a legitimate emergency use; visibility is the goal). Document the three vars in `.env.example`'s optional block (coordinates with Plan 08 — whichever lands second reconciles).
...
- **Live read-only:** after deploy, one legitimate admin action performed by the operator confirms no lockout; Workers logs show no `admin_allowlist_missing`.
- **Live mutation:** none (except the possible pre-sequenced secret set, gated above).
