...
- [ ] **Production host = BOTH custom domains.** `wrangler.jsonc` routes the Worker for **`anna-ciok.studio`** *and* **`www.anna-ciok.studio`** — the production-host check must treat **both** as production (a host-suffix match on `anna-ciok.studio` covers both; do not hardcode only the apex). Otherwise a valid Cloudflare Access JWT hitting the omitted host bypasses the `ADMIN_ALLOWED_EMAILS` deny.
- [ ] **Ordering (critical): evaluate the production-host missing-allowlist deny BEFORE honouring `STUDIO_ADMIN_LOCAL_BYPASS`.** If the bypass is checked first, a bypass flag mistakenly set on a production deployment would skip the 403 and preserve the exact M-6 fail-open the task closes. The precedence must be: production host + empty `ADMIN_ALLOWED_EMAILS` → **403**, regardless of the bypass flag; the local bypass is honoured **only** on a non-production hostname (`STUDIO_ADMIN_LOCAL_BYPASS` already additionally requires a private/loopback host, so a public alternate host cannot activate it — but the ordering must still not depend on that). Non-prod/dev keep current behaviour so preview doesn't break.
- [ ] Failing tests for the ordering, **one per production host**: `anna-ciok.studio` and `www.anna-ciok.studio`, each with `STUDIO_ADMIN_LOCAL_BYPASS` set + missing `ADMIN_ALLOWED_EMAILS` → **403** (proves neither host + bypass re-opens the fail-open).
