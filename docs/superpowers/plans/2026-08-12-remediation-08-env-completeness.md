...
- [ ] **Failing test first.** `scripts/env-example-completeness.test.ts`:
  ...
  - Parse `.env.example` for defined names (`^\s*#?\s*([A-Z0-9_]+)=` — commented-out entries count as documented).
  - Assert every **required** key from the type file appears in `.env.example`. Allowlist (with justification comments) only for keys genuinely never user-provisioned (e.g. bindings like `FULFILMENT_QUEUE`, `PRINT_ASSETS`, `ASSETS`, `WORKER_SELF_REFERENCE` — binding names live in `wrangler.jsonc`, not env files; also `NEXT_PUBLIC_APP_VERSION` which is build-inlined).
...
- [ ] Add the two required entries to `.env.example` with comments matching neighbours (mode-specific PMC id, `wrangler secret put` instruction, checkout-fails-closed note; dedicated HMAC secret, never reuse others — mirror the audit/AGENTS.md language). Add the three `ADMIN_*` vars as a commented **optional, local-only** block referencing `src/lib/admin/clients.ts`.
