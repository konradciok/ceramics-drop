...
Expected: all 28 assertions pass (`# tests 28` / `ok 1` … `ok 28`, no `not ok` lines).
...
**Interfaces:**
- Consumes: `variantKey`, `assetPxFor`, `PRODIGI_SKU_MAP` from `@/lib/print-cart` (exact shapes confirmed: `variantKey(sel: {size,framed,mount,frameColour}): string`; `assetPxFor(entry: Pick<ProdigiSkuEntry,'printAreaPx'|'assetPx'>): {w:number;h:number}` — returns `entry.assetPx ?? entry.printAreaPx`, the real master-asset pixel dimensions, which diverge from `printAreaPx` for exactly one entry (`30x40:true:false:black`, Decision 2026-08-03); `PRODIGI_SKU_MAP: Record<string, {sku: string; printAreaPx: {w: number; h: number}; assetPx?: {w: number; h: number}}>`), `publish_product_revision` RPC (Task 2), `loadProductResponse` (Task 6), `claimIdempotencyKey`/`completeIdempotencyKey`/`releaseIdempotencyKey` (Task 4).
...
**Interfaces:**
- Produces: `decide_print_proof(p_asset_id uuid, p_product_id text, p_action text, p_expected_revision integer, p_actor_email text) returns jsonb` (`{ok: true, productId: text}`).
...
Run: `supabase test db`
Expected: 9 assertions pass, plus the existing 28 from `cms_api_products.sql` and 11 from `guarded_product_status.sql` (etc.) still pass.
...
- [ ] **Step 4: Add the 3 new docs to the docs index**

`docs/cms-api-data-model.md` (Task 1), `docs/cms-api-integration-environment.md`
(Task 12), and `docs/cms-api-s1-handoff.md` (this task) are all new files
under `docs/` — per `AGENTS.md`'s own "Context map" convention and
`docs/README.md`'s "Lifecycle rule", every doc under `docs/` must be
indexed there. In `docs/README.md`, add one row per file to the
"## Guides & runbooks" table (same table `orders-cli.md` etc. live in):
...
```bash
git add AGENTS.md docs/cms-api-s1-handoff.md docs/README.md
git commit -m "$(cat <<'EOF'
...
the brief's Definition of Done. Also indexes the 3 new docs/cms-api-*.md
files in docs/README.md per its own documented convention.
