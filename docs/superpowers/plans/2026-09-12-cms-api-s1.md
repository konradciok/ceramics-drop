...
Expected: all 22 assertions pass (`# tests 22` / `ok 1` … `ok 22`, no `not ok` lines).
...
- Produces: `claimIdempotencyKey(supabase, operation, key, requestBody): Promise<IdempotencyClaim>`, `completeIdempotencyKey(supabase, operation, key, status, body): Promise<void>`, `releaseIdempotencyKey(supabase, operation, key): Promise<void>`, exported type `IdempotencyClaim = { kind: 'run' } | { kind: 'replay'; status: number; body: unknown } | { kind: 'in_progress' } | { kind: 'key_reuse' }`. Later tasks wrap every idempotent handler as: `claim → if not 'run', short-circuit; else run the handler in a try/catch → completeIdempotencyKey on success, releaseIdempotencyKey on throw (so a crashed attempt can be retried immediately, mirroring src/lib/webhook.ts's releaseLease()) → return the handler's response`.
...
Note for the implementer: the `update(...).eq().eq().eq(...)` chain in `fakeSupabase` models three real `.eq()` calls (`operation`, `idempotency_key`, `processing_started_at`) — if the real `claimIdempotencyKey` implementation's chain shape differs once written, adjust the fake's nesting to match rather than changing the production code's query shape.
...
export const productsCreateRoute: RouteDef = {
  ...
  handler: async (req, env, _params, ctx) => {
    ...
    if (!validated.ok) {
      await releaseIdempotencyKey(ctx.supabase, 'products:create', idempotencyKey);
    ...
    try {
      ...
      await completeIdempotencyKey(ctx.supabase, 'products:create', idempotencyKey, 200, product);
...
export const productsDuplicateRoute: RouteDef = {
  ...
  handler: async (req, env, params, ctx) => {
    ...
    try {
      ...
      if (!source) {
        await releaseIdempotencyKey(ctx.supabase, 'products:duplicate', idempotencyKey);
        return errorResponse('NOT_FOUND', `Product ${params.id} does not exist.`, 404, ctx.requestId);
      }
      ...
      await completeIdempotencyKey(ctx.supabase, 'products:duplicate', idempotencyKey, 200, duplicate);
...
describe('productsCreateRoute', () => {
  beforeEach(() => {
    vi.mocked(idempotency.claimIdempotencyKey).mockResolvedValue({ kind: 'run' });
...
export const publicationPostRoute: RouteDef = {
  ...
  handler: async (req, env, params, ctx) => {
    ...
    try {
      const product = await loadProductResponse(ctx.supabase, env, params.id);
      if (!product) {
        await releaseIdempotencyKey(ctx.supabase, 'products:publication', idempotencyKey);
        return errorResponse('NOT_FOUND', `Product ${params.id} does not exist.`, 404, ctx.requestId);
      }
      ...
      await completeIdempotencyKey(ctx.supabase, 'products:publication', idempotencyKey, 200, updated);
