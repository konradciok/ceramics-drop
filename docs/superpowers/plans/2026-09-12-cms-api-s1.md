...
Expected: all 20 assertions pass (`# tests 20` / `ok 1` … `ok 20`, no `not ok` lines).
...
const ceramicDraftSchema = z
  .object({
    ...
    priceGbp: z.number().int().nonnegative(),
...
Run: `npx vitest run src/server/cms-api/`
Expected: all tests pass (adds validation: 8, products-create: 7, products-save: 5, products-duplicate: 4 to the running total).
