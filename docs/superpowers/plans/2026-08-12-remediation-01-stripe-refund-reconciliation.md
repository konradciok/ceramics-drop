...
- `npm run orders -- reconcile-refunds` (dry-run by default) lists fully-refunded Stripe payments whose DB order is not fully converged, and with `--confirm <order-id>` converges one order. Convergence is **not** claimed complete until *all four* `releaseSale` side effects are accounted for: order-status CAS, `piece_state` relist, Prodigi print-fulfilment cancel, and GA4 revenue reversal. Any side effect the CLI cannot perform offline (Prodigi cancel, GA4 reversal) is reported as **required follow-up** and the order stays **partially-converged** (still surfaced by subsequent dry-runs) until the operator confirms it — the dry-run must never read empty while a refunded order still has an active Prodigi job or unreversed analytics. `--skip-relist` is for pieces that should stay off sale (e.g. damaged-in-transit).
...
- [ ] Add a `reconcile-refunds` command to `scripts/orders-cli.ts`:
  ...
  - Join to `orders` by `payment_intent_id`; report every order that is **not fully converged** — i.e. status ≠ `refunded`, **or** any `piece_state` row for the order still `sold`, **or** (print orders) an active `prodigi_orders`/`fulfilment_jobs` row, **or** analytics not yet reversed. An order drops off the report only when every side effect is done. This is deliberately stricter than "status = paid" so the dry-run cannot read empty while a refunded order still has an active Prodigi job (PII-redacted like the rest of the CLI).
  - With `--confirm <order-id>`: perform the side effects the CLI *can* do offline — CAS `paid→refunded` (and `pending→refunded`), relist `piece_state` rows `sold→available` scoped to `order_id` — then, for the side effects it **cannot** safely do offline, emit an explicit **REQUIRED FOLLOW-UP** block naming each: (a) Prodigi cancel via `cancelPrintFulfilment` (needs the Workers env — do not auto-run from the CLI) and (b) GA4 revenue reversal (no marketing context offline). The command exits reporting the order as **partially-converged** whenever any follow-up remains, so the next dry-run still lists it. It does **not** claim `releaseSale` parity — it is a bounded repair with an explicit remainder. `--skip-relist` leaves `piece_state` untouched (operator decides whether a refunded piece returns to sale — e.g. damaged-in-transit must NOT relist) and records that as a deliberate, logged decision, not a silent skip.
  - Non-prod block without `--allow-nonprod`, same as every other mutation in the CLI.
- [ ] Tests (mocked stripe/supabase factories): dry-run reporting; `--confirm` performs exactly the two CAS-scoped updates **and** emits the REQUIRED FOLLOW-UP block for Prodigi cancel + GA4 when applicable; an order with an active `prodigi_orders` row is reported as partially-converged and re-appears in a subsequent dry-run; `--skip-relist` performs only the order CAS and logs the decision; a partial refund is excluded.
