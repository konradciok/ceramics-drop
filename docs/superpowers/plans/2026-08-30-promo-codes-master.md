...
- No refund clawback of a redemption (a refunded order's redemption stays counted; documented operator behavior).
...
2. **Discount semantics:** applies to the **merchandise subtotal only, never shipping**. Two kinds: `percent` (integer 1–100, `discount = floor(subtotalMinor * percent / 100)`) and `fixed` (explicit per-currency minor-unit amounts `amount_pln/amount_eur/amount_gbp`, clamped to subtotal). Final clamp: if `subtotal − discount + shipping < STRIPE_MIN[currency]` (PLN 200 / EUR 50 / GBP 30 minor units), reduce the discount so the charge equals the minimum. One code per order, no stacking.
...
5. **Redemption lifecycle:** claim `pending` atomically at checkout via RPC `claim_promo_redemption(p_promo_id, p_order_id)` (re-entrant for the same order id — checkout replays must not double-count); `markPaid` → `redeemed` (idempotent guarded UPDATE, throws on DB error so Stripe retries); `releaseHold` + the cron expiry sweep → `released`. Refund: stays `redeemed`.
