...
- [ ] **Pending-consumer guard must protect the *first irreversible mutation*, not the final status update.** A `refund_pending_at IS NULL` predicate on the *final* `pending→expired` update is **too late** in the current flows — the irreversible action already happened before it:
  ...
  - **Transition to terminal `expired` only after *both* `cancelIntent` and piece release succeed — and fence the terminal CAS to the claimant's own lease token.** The terminal write must be scoped to the exact `expiry_claim_at` this consumer claimed: `update orders set status='expired', expiry_claim_at=null where payment_intent_id=$pi and status='pending' and expiry_claim_at=$claimedAt returning id`. A `status='pending'`-only predicate is **not** enough: a lease can expire while claimant A is still waiting on Stripe / piece release, claimant B then acquires a **new** lease, and A — finishing late — would otherwise finalize an order it no longer owns. If the fenced CAS returns **zero rows**, A has lost ownership: it must **stop and not overwrite** the newer claimant's state (no `expired` write, no error that would double-act) and let B or a later retry converge the order. (This is the same claim-scoping discipline as L-4's `.eq('processing_started_at', claimedAt)` on the webhook ledger.) On any side-effect error, likewise leave the order `pending` (do not go terminal) so reconciliation retries — the current recovery contract, now refund-pending-safe **and** lease-fenced.
...
- **New (enumerated — the release gate requires every one, do not collapse to a round number):**
  ...
  Total **≥ 29 task-level cases + N** (one per L-7 catch); the gate is the enumerated list, not the number.
