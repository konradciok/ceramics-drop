...
| Stripe webhook events (in) | Stripe | `stripe/webhook/route.ts` | maszyna stanów orders/piece_state | CAS per status; **brak ledgera event-id** (F-18) | at-least-once; kroki idempotentne przez CAS/claimy — z wyjątkiem konwersji serwerowych, gdzie dedup jest delegowany do vendorów (F-05) |
...
- **Rekomendacja:** kolumna `orders.conversions_sent_at` z claim-once CAS (dokładnie wzorzec `sendEmailOnceWithClaim`, `route.ts:39-87`) — wysyłka co najwyżej raz na claim, retry tylko po releasie clama. Świadomy trade-off do udokumentowania: claim nie odróżnia „vendor przyjął" od „crash tuż po wysyłce/przed releasem" — zamienia ryzyko duplikacji na wąskie okno utraty (mitygacja: dedup vendora zostaje jako druga linia, a stuck claim jest wykrywalny i domykalny ręcznie).
...
- **Skutki:** sekretne tokeny lądują w payloadach eventów wysyłanych do GA4 i Mety, a stamtąd w raportach, eksportach (np. BigQuery) i logach vendorów; niezależnie od analityki trafiają też do historii przeglądarki i potencjalnie do nagłówka `Referer`. Każdy z dostępem do raportów GA4 może odczytać ważny link umożliwiający zakup zarezerwowanej pracy. Ten sam mechanizm redakcji chroni już `order` — pominięcie `sale` wygląda na przeoczenie przy późniejszym dodaniu featuru.
...
- **Nic — w tym projekcie ssGTM nie ma wystarczającego ROI.** Argumenty: (1) sklep ma dokładnie **dwa serwerowe cele marketingowe** (Meta CAPI, GA4 MP) — pozostałe integracje serwerowe (Stripe, Resend, InPost, Prodigi, Supabase) to operacje biznesowe, nie kandydaci do warstwy tagów — oba już zaimplementowane bezpośrednio, konsent-gated, z hashowaniem i dedupem — ssGTM dodałby ~120 zł+/mies. kosztu utrzymania kontenera (App Engine/Cloud Run/Stape), nowy komponent do monitorowania i zero nowych możliwości; (2) wolumen (~125 unikatów + printy) jest o rzędy wielkości poniżej progu, przy którym first-party serving i odciążenie przeglądarki mają znaczenie; (3) zespół jest jednoosobowy/agentowy — każda dodatkowa platforma to realny koszt. Jedyny scenariusz zmiany oceny: dojście 2–3 kolejnych serwerowych odbiorców konwersji (TikTok, Pinterest, Google Ads Enhanced Conversions) — wtedy ssGTM zaczyna spłacać złożoność.
