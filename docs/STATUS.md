...
| GitHub Actions | Repo is public (unlimited minutes); CI + E2E are required checks on `main`. From the `docs/github-actions-audit.md` operator checklist: `PRINT_ASSET_TOKEN_SECRET` + `PRODIGI_API_KEY_SANDBOX` repo secrets set, `prodigi-contract-smoke` dispatched (success), `PRINT_SMOKE_STRICT=true`. Remaining: set `PRODIGI_SMOKE_STRICT=true` | 2026-08-07 |
...
| Versioning | release-please active; current series pre-1.0 (`0.x`), v0.12.0 released, v0.13.0 release PR open (#217) | 2026-08-07 |
