...
| Fine-art prints | Storefront + Prodigi pipeline live; **39 designs are active** across nine fixed-name collections. `fap029` and `fap037` are archived duplicates. Stable `fapNNN` IDs remain unchanged; `config/print-catalog-curation.json` is the source of truth for collection membership, storefront numbering, and retired designs. Retired assets remain available for historical fulfilment. `prodigi-contract-smoke` dispatched successfully 2026-07-23 | 2026-08-28 |
