...
Read this alongside `supabase/migrations/20260915120000_cms_api_collections.sql`. A collection is a plainer resource than a product — no ceramic/print structural columns of its own, no `status` state machine, no DELETE — but it follows the same draft/publish/restore skeleton as products.

- **`collections`** — `id text primary key` (the `col_<10 hex>` token minted by `POST /v1/collections`), `published_revision` (new nullable column, composite-FK'd to `collection_drafts(collection_id, revision)`), `published_at`, timestamps. There is no status column: a collection is either unpublished (`published_revision IS NULL`) or published at whatever revision that column holds — no hide/archive state like products.
- **`collection_drafts`** — one immutable row per saved draft, `(collection_id, revision)` unique, `revision` starting at 1 and only ever incrementing, exactly like `product_drafts`. `payload` is the `{name, fields}` object directly (no wrapper key, unlike products' `{draft: ...}`). `Resource.revision` in API responses is `max(collection_drafts.revision)` for that collection (the latest saved draft, whether or not it's published); `Resource.publishedRevision` is `collections.published_revision` verbatim. Nothing here is ever deleted or rewritten — restore re-saves an old payload as a *new* revision, same as this doc already promised above for products.

`create_collection_with_draft()` inserts `collections` plus its first `collection_drafts` row (revision 1) in one transaction; a `p_id` collision surfaces as a `23505 unique_violation` on `collections_pkey` for the handler to catch and retry with a new id, same convention as `create_product_with_draft`.

`save_collection_draft()` and `restore_collection_draft()` both raise `collection_not_found` if the id doesn't exist, and `revision_conflict` (detail `currentRevision=<n>`) if `p_expected_revision` doesn't match `max(collection_drafts.revision)`. Restore additionally raises `source_revision_not_found` if `p_source_revision` has no matching `collection_drafts` row; on success it copies that old revision's payload into a brand-new revision (`current + 1`) and never touches `collections.published_revision` — restoring an old draft does not publish it.

`publish_collection_revision()` runs the same `collection_not_found`/`revision_conflict` checks, plus `draft_required` when `p_expected_revision = 0` (no draft was ever saved — same convention as products). Two checks have no product precedent, both against the current draft's `payload->'fields'`: `missing_polish` when no field has `locale = 'pl'`, or any `pl` field's `value` is blank after trimming; `product_ref_invalid` (detail `invalidIds=<comma-joined ids>`) when a `productIds`-type field's CSV `value` contains an id that doesn't exist in `products` (an empty CSV — no products in the collection — is valid, not an error). On success it only stamps `published_revision`/`published_at` on `collections`; unlike `publish_product_revision` there is no structural table (variants/media) to replace.

**Audit actions:** every write above stamps a `catalog_audit_log` row scoped by the new nullable `collection_id` column (`product_id` stays null on these rows, and vice versa on existing product rows — `catalog_audit_log_product_or_collection_check` enforces exactly one of the two is set). `create` and `save` both write `'draft_saved'`; `publish` writes `'published'`; `restore` writes `'restored'`.
