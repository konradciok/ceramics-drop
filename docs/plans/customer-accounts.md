...
|---|---|---|
| `/api/auth/login` | POST (form) | Fail-closed 404 unless `authEnabled`; rate-limited (clone the `/api/returns` limiter pattern); validates `provider ∈ {google, apple}`; sanitizes `next` with the **URL parser, not a regex** — reject backslashes/control characters, resolve `new URL(next, SITE_URL)`, require the trusted origin, keep only `pathname + search` (a naive `^\/(?!\/)` check would pass `/\evil.example`, which browsers normalize into a protocol-relative external redirect) — and stashes the sanitized value in a short-lived httpOnly cookie; `signInWithOAuth({ provider, options: { redirectTo: <origin>/api/auth/callback, skipBrowserRedirect: true } })` → 303 to the provider URL (PKCE verifier cookie written httpOnly). |
...
- **`src/lib/auth/supabase-server.ts`** — two `createServerClient` factories (from `next/headers` cookies; from request/response for middleware), each implementing **both** cookie-adapter callbacks per the Supabase SSR contract: `getAll()` reads request cookies, `setAll()` writes refreshed cookies *and* the library-supplied anti-cache headers onto the response (omitting `setAll` causes refreshes that never persist — random logouts). The `next/headers` factory try/catch-swallows `setAll` only in pure-RSC render contexts where headers are immutable — which is why middleware owns the `/konto` refresh. Both pin `cookieOptions` (httpOnly/secure/lax/path) + `cookieEncoding: 'base64url'`.
...
- Verification is **local jose JWT verification** everywhere (fail-closed: any error = anonymous); Supabase is contacted only to mint/refresh/revoke sessions. Local verification is explicitly a **fast path bounded by the access-token TTL** — it cannot observe server-side revocation, so a revoked-but-unexpired token remains accepted for up to 1 h (§9 on why that is acceptable here). Same-browser sign-out is immediate regardless, because the cookies are deleted.
...
- **OAuth:** PKCE code flow end-to-end; provider secrets live only in Supabase's dashboard (never in our env/bundle); Apple form_post terminates at Supabase; our callback validates via `exchangeCodeForSession` (code + verifier bound); `next` redirect targets are URL-parser-validated against the site origin (backslash/control-character rejection, origin comparison via `new URL(next, SITE_URL)`, only path+query ever redirected to — enforced at `/api/auth/login` and again at the `/api/auth/callback` fallback; open-redirect proof); login endpoint is rate-limited and POST-only (login-CSRF resistant); provider `?error` responses never mint cookies.
- **Session management:** httpOnly + Secure + SameSite=Lax cookies (tokens are **never** readable by JS — stricter than the Supabase default browser setup, and it makes the Report-Only CSP a non-blocker); refresh-token rotation with 10 s reuse window; sign-out deletes the cookies (immediate for that browser) and revokes the session family server-side (`scope: 'local'`) — documented trade-off: an already-issued access token verified locally elsewhere stays valid until `exp` (≤1 h), the same stateless-JWT acceptance the Cloudflare Access admin gate already makes, and bounded to read-only order-history exposure; if instant revocation ever becomes a requirement, account-page reads switch to `getUser()` (one network call per render) with no other design change; JWT verification checks signature (asymmetric JWKS), `iss`, `aud`, `exp` — mirroring `src/lib/admin/access.ts`; sessions die cleanly if the secret is removed (kill switch).
