...
- Deferred to follow-up: reactively re-checking consent if the shopper accepts the cookie banner while already focused on the address field. Checking once on mount/first interaction (matching `AuthAnalytics.tsx`'s precedent) is sufficient for v1; a live consent-change listener is a UX polish item, not a blocker.
...
- **Manual provisioning dependency**: a Google Cloud project with billing enabled, the Maps JavaScript API turned on, and a browser key restricted by HTTP referrer and API-restricted to the Maps JavaScript API must exist before this can go live (see U5). This is an out-of-repo operational step this plan can't automate.
...
- **Approach**: a comment block noting it's a build-time Workers Build env var (not a wrangler secret), requires a Google Cloud project with billing enabled and a browser key restricted by HTTP referrer plus API restriction to the Maps JavaScript API, and is blank by default. Provisioning checklist (out-of-repo, tracked here so it isn't dropped): create the Cloud project, enable the Maps JavaScript API and billing, generate two referrer-restricted keys (production origin, local-dev/preview origin — see Risks & Dependencies), and set a budget alert and/or daily quota cap on the project as the mitigation for a scraped key being replayed outside the browser's referrer check.
