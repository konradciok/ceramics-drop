...
- CAS discipline: every mutating endpoint's body carries `expectedRevision` (the job's `PrintAssetJobRow.asset_revision`-derived contract `revision` — NOT `attempts` — same idiom as retry), checked before any write.
