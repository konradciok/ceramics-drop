...
11. Ustanowić baseline GMC/Bing/CrUX i przeglądy 28/56/90 dni. GA4 baseline już potwierdzony 2026-08-31 (§13.3, §13.7), a GSC domain-verification potwierdzona przez DNS TXT (§13.5) — pozostaje GMC, Bing i CrUX oraz regularny cadence przeglądu.
...
Sentry (`anna-ciok-studio`, region `us.sentry.io`) — `search_issues` z `is:unresolved` i osobno `is:resolved` na oknach 24h/90d, oraz zapytanie tekstowe `timeout`: **zero wyników za każdym razem**. Supabase `get_advisors(type: performance)` nie pokazuje nic, co tłumaczyłoby wolne zapytania (tylko nieużywane/duplikowane indeksy na małych tabelach pomocniczych). ~~Wniosek: 504 najprawdopodobniej powstaje na granicy Workers/edge (limit CPU/wall-clock albo opóźnienie sieciowe do Supabase) zanim aplikacja zdąży to zalogować do Sentry — sam brak pokrycia monitoringu jest osobnym, wartym naprawienia faktem.~~ **Ta hipoteza jest obalona — zob. aktualizację poniżej.**

#### Aktualizacja 2026-09-02 — rozstrzygnięcie (poprzedni org Sentry `anna-ciok-studio` był pusty; poprawna organizacja to `y9608071l-anna-ciok`, `us.sentry.io`, projekt `ceramics-drop` — potwierdzone wcześniej, 2026-09-02 SEO-triage PR #276)

**Metoda:** wykorzystano `wrangler tail` (token OAuth ma scope `workers_tail:read`, którego brakowało dla GraphQL) do potwierdzenia, że żywy ruch nie generuje 504 na poziomie Workera, plus jednorazowy Cloudflare API token od właściciela (`Account/Zone Analytics:Read`) do właściwych zapytań GraphQL — patrz `Cloudflare` w tabeli dostępów §13 (token użyty tylko w pamięci procesu, nigdy niezapisany w repo).

**H1 (Worker sam generuje 504) — OBALONA.** `workersZoneInvocationsAdaptiveGroups` (zone `df154a46a71277a8b5b4a9e3d9af23ad`) dla `2026-09-01T13:00Z→2026-09-02T13:00Z`: **9996 inwokacji Workera, 0 z `httpResponseStatus 504`** (7576 `200`, 1937 `404`, reszta redirecty/204, plus 11 pomijalnych `canceled`). Zero `exceptions`, zero `exceededCpu`. `wrangler tail` na żywym ruchu (~20 zdarzeń, w większości boty: SentryUptimeBot, AhrefsBot, ClaudeBot, bingbot, skaner `Faraday`) — wszystkie `outcome:"ok"`, zero `status:504`.

**H2 (504 generowany na edge, bez inwokacji Workera) — POTWIERDZONA, ROOT CAUSE.** Ten sam okno 24h w `httpRequestsAdaptiveGroups` (raw edge log): 17674 requestów, 1785 z `edgeResponseStatus 504` (**10,10%** — identyczne z bazową stopą z 2026-08-25/31). Rozkład po `requestSource`:

```text
edgeWorkerFetch:      3157 total,    0×504
eyeball:              12385 total,   0×504
inBrowserChallenge:    155 total,    0×504
earlyHintsCache:       1785 total, 1785×504   ← 100% tej grupy, i 100% wszystkich 504
```

Zweryfikowane niezależnie na `2026-08-31T00:00Z→2026-09-01T00:00Z` (dzień pierwotnego pomiaru z §13.2 powyżej): 23586 total, 2106×504 (8,93%), `earlyHintsCache` = 2106/2106 = znowu dokładnie 100%. Dla obu okien: `originResponseStatus` = `0` na wszystkich 1785/2106 wierszy, `cacheStatus` = `miss` na wszystkich, `clientRequestHTTPProtocol` = `UNK` na wszystkich, metoda zawsze `GET`, rozkład ścieżek płaski i niesegmentowy (501 różnych ścieżek w 24h, home/PDP wszystkich kategorii i lokali/`/sklep`/`/showroom`/`/gallery`/`/polityka-prywatnosci` — stąd też `/o-studiu`-podobne statyczne strony w oryginalnej próbce z §13.2, punkt 5 briefu diagnostycznego). Geografia (US 599, ES 535, PL 363, TR 215, FR 42…) i colo (MAD/ATL/WAW/MXP…) to geografia realnego ruchu do strefy, nie coś specyficznego dla 504 — bo `earlyHintsCache` lookup wykonuje się dla każdego requestu niezależnie od pochodzenia.

**Wyjaśnienie mechanizmu:** `requestSource: earlyHintsCache` + `originResponseStatus: 0` to udokumentowane, wewnętrzne zachowanie Cloudflare ([Early Hints docs](https://developers.cloudflare.com/cache/advanced-configuration/early-hints/), [„504 responses with origin status 0 in Logpush"](https://developers.cloudflare.com/logs/faq/504-origin-status-0/)): gdy zone-level `early_hints` jest `on` (potwierdzone: `GET /zones/{id}/settings/early_hints` → `"value":"on"`), Cloudflare przy każdym requeście wykonuje wewnętrzny lookup „czy mam scache'owane nagłówki Early Hints dla tego URI" — `200` na cache hit, `504` na cache miss. To nigdy nie dotyka origin/Workera i nie jest realnym błędem dla użytkownika; dashboard Cache/HTTP Analytics filtruje `requestSource: eyeball`, więc te wiersze są tam niewidoczne — widać je tylko w surowym GraphQL/Logpush, którego użył oryginalny pomiar z 2026-08-31. `src/app/[locale]/layout.tsx` emituje na każdej odpowiedzi nagłówek `Link:` (hreflang alternates, potwierdzone `curl -I` na produkcji) — to prawdopodobnie to, co skłoniło Cloudflare do aktywnego sprawdzania cache'u Early Hints na każdym requeście, mimo że aplikacja nigdy nie emituje `rel=preload`/`rel=preconnect` i nigdy świadomie nie korzystała z Early Hints (grep repo: brak). Cache więc nigdy się nie zapełnia i **każdy** taki lookup jest miss → 504.

**Wniosek:** brak realnej awarii produkcyjnej. Baza „10%" z §1 i z tego rozdziału to w całości ten wewnętrzny, benign mechanizm — nie ma tu wpływu na Googlebota, GSC Crawl Stats ani realnych użytkowników (zob. retrakcję ryzyka GSC w SEO-016 poniżej). Nie przypisywać winy PR #266 ani żadnemu innemu konkretnemu merge'owi — konfiguracja `early_hints:on` jest niezależna od aplikacji i zgodna z tym, że baza sprzed tygodnia (2026-08-25, 9,9%) już istniała.

**Rekomendacja (operacyjna, nie PR):** wyłączyć `early_hints` w Cloudflare dashboard (Speed → Optimization → Content Optimization) — aplikacja nigdy świadomie nie używała tej funkcji, więc nie ma z niej żadnej korzyści, a wyłączenie usuwa ten szum u źródła zamiast filtrować go downstream. Alternatywa bez zmiany configu: każdy przyszły monitoring/alerting oparty o `httpRequestsAdaptiveGroups` musi filtrować `requestSource: "eyeball"` (tak jak robi to natywny dashboard Cloudflare), inaczej powtórzy ten sam fałszywy alarm.

**Weryfikacja po wyłączeniu (do zrobienia przez właściciela):** `httpRequestsAdaptiveGroups` filtrowane po `requestSource:"earlyHintsCache"` dla dowolnego 24h okna po zmianie powinno zwrócić 0 wierszy; ogólna `edgeResponseStatus:504` rate powinna spaść do ~0% (poprzednio jedyne realne 5xx w próbce to 2×502/2×530 z 24623, czyli <0,02%).
...
Dwa niezależne systemy (transakcyjna baza i GA4) zgadzają się: brak sprzedaży od `2026-07-03` (59 dni przed datą audytu), zero purchase events w GA4 w oknie ostatnich 30 dni (`--days 30`; okno nie obejmuje 1–2.08, więc to nie jest dowód dla całego sierpnia), i **0 kupowalnych wierszy `piece_state` na 126 istniejących** (nie mylić ze 125 aktywnymi produktami ceramicznymi z §13.3 — to dwa różne liczniki: 126 to wszystkie wiersze `piece_state`, w tym co najmniej jeden bez odpowiadającego aktywnego produktu). Efekt jest ten sam: **cały aktywny katalog ceramiki jest dziś niekupowalny.** To nie jest klasyczny finding SEO — to kontekst, który zmienia, co w §8 ma sens robić najpierw (stąd SEO-018 w §4 i pozycja 0 w „Najważniejsze następne działania”).
...
**Aktualizacja 2026-09-02:** poprawna, aktywna organizacja to `y9608071l-anna-ciok` (`de.sentry.io`), nie `anna-ciok-studio` (pusta — patrz PR #276). ~~Przy tym zastrzeżeniu: ~10% error rate przechodzące bez żadnego pasującego zdarzenia sugeruje, że pokrycie najprawdopodobniej kończy się na granicy aplikacji, nie edge/Workers~~ — ta hipoteza jest obalona przez §13.2: nie ma tu żadnej luki w pokryciu do naprawienia, bo te ~10% nigdy nie były realnymi błędami aplikacji (wewnętrzny mechanizm Cloudflare Early Hints, zob. §13.2) — Sentry słusznie milczał, bo Worker nigdy nie był wywoływany dla tych requestów.

Poza brakiem jakiegokolwiek zdarzenia korelującego z (nieistniejącym) incydentem 504, w organizacji Sentry istnieje zweryfikowany projekt (`ceramics-drop`) zgodny z `docs/analytics-stack.md` — ale samo istnienie organizacji **nie dowodzi**, że wdrożony na produkcji runtime aktualnie wysyła do niej zdarzenia; tego nie sprawdzono bezpośrednio (wymagałoby to np. wygenerowania testowego błędu po deployu albo odczytu `SENTRY_DSN` z konfiguracji Workers, czego nie zrobiono w tej weryfikacji). To pozostaje osobnym, wartym sprawdzenia pytaniem — niezależnym od SEO-016.
