...
| [`audits/`](audits/) | reference | Domain audits (analytics, event system, …) — incl. `backend-audit-2026-08-12.md` (the Aug-2026 backend audit) + its `-verification.md` live-gate log, `2026-09-11-production-launch-audit.md` (pre-launch audit: 7 fixes shipped, 3 operator launch checks open), and `2026-09-25-dead-code-audit.md` (repo-wide dead-code audit: 14 confirmed-dead items incl. the retired returns-flow cluster, 8 probably-dead, 0 dead endpoints/deps; also documents ~9 stale `AGENTS.md`/doc claims and ~20-25 shipped plans still outside `archive/`) |
