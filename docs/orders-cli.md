...
- `prodigi-env-check [--since ISO8601]` — Stripe livemode vs `PRODIGI_ENV` drift guard (needs `STRIPE_SECRET_KEY` + Supabase creds). For every `fulfilment_jobs` row (unbounded by default — Prodigi order volume is currently near-zero; `--since` optionally bounds by `created_at`) it retrieves the order's Stripe PaymentIntent and compares `.livemode` against the row's persisted `prodigi_env`: a live payment fulfilled under a non-`live` env, or a test payment fulfilled under `live`, both exit `4` with code `prodigi_env_mismatch`, listing every offending row. This is exactly how the 2026-09-02 sandbox misfulfilment incident (order `63445e00-b433-45bc-a42e-d723c273c091`) went undetected in production — Prodigi's sandbox fully simulates the happy path, so nothing errored and no alert fired at the time. No `--allow-nonprod` guard needed, this is read-only.
...
5. Run `npm run orders -- prodigi-env-check`. Against production data prior to remediation, expect exit `4` flagging order `63445e00-...` (`problem: "live_payment_nonlive_env"`); after `PRODIGI_ENV` is corrected and no further mismatched rows exist, expect exit `0` with `anomalies: []`.
