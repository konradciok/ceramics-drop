...
The **Meta Pixel tags** (`ACC - Meta base` + `ACC - Meta PageView`/`ACC - Meta standard events`/`ACC - Meta SiteEngagement`, all `ad_storage`) are **UI-managed** — the official Meta Pixel template is imported once from the gallery and trigger/consent-wired via one-off API calls (the GTM API cannot import Community-Gallery templates), exactly like `Microsoft Clarity - Official`. They are not created by `gtm-api.mjs`; they persist in the container and are captured in the version export.
...
3. Confirm every consent-relevant tag still shows `consentSettings.consentStatus: "needed"` in the export, gated on `analytics_storage` (GA4) or `ad_storage` (Meta) as appropriate, **and** that `ACC - GA4 config`, `ACC - Meta base`, and `Microsoft Clarity - Official` each fire on two triggers (`ACC - Initialization`/its own base trigger, plus `ACC - Consent Update`) — this is what lets them recover if a visitor accepts consent mid-session instead of on load. Post-v15 the GA4 tags are native (`ACC - GA4 config` `googtag` + `ACC - GA4 event` `gaawe`, gated via `consentNeeded()` in `scripts/gtm-api.mjs`); the Meta Pixel tags (`ACC - Meta *`) and `Microsoft Clarity - Official` are **UI-managed** imported template tags **not** managed by `gtm-api.mjs`, so each needs its consent status set by hand in the UI (Meta → `ad_storage`, Clarity → `analytics_storage`).
...
**Plan 6 native-tag migration (F-03/F-04) — `scripts/gtm-api.mjs` landed; container publish to v15 still pending.** The script now builds native GA4 tags — `ACC - GA4 config` (`googtag`, `send_page_view:false`) + `ACC - GA4 event` (`gaawe`, `eventName={{Event}}`, `sendEcommerceData`) — instead of the four Custom-HTML tags/bridges (retires the `google_tag_manager[…].dataLayer.get()` reverse-scan, **F-03**), and drops the dead `fbq('set','userData')` advanced matching (**F-04**, now on the Meta event tag's user-data field). The N-1 `page_location` redaction is carried forward from v14's inline `redactLocation()` into one fresh-evaluated `Page Location - redacted` custom-JS variable referenced by both native GA4 tags (subsumes the v14 config seed **and** the bridge `gtag('set')` refresh). Meta becomes the official gallery Pixel template (UI-imported + one-off API trigger/consent wiring, like `Microsoft Clarity - Official`). Two review-driven details to confirm at the Preview gate: `ACC - GA4 config` fires **`unlimited`** (not `oncePerLoad`) so the `ACC - Consent Update` re-fire actually recovers after a mid-session Accept (set the UI-managed `ACC - Meta base` the same way); and GA4 client-side user-provided data is **not** sent as a user property (that would miss enhanced-conversions matching and breach Google's PII policy) — server-side GA4 MP already carries hashed match data, so wire native UPD only if explicitly wanted. **The live container is still v14** — publishing v15 needs the operator-only steps that cannot run from CI: `npm run gtm:setup` against the draft, the GTM-UI Meta-template gallery import + tag config, a GTM Preview / GA4 DebugView / Meta Test-Events parity gate, then create + publish the version and re-export to `docs/GTM-NPHLG9NR_v15.json` (removing `_v14.json`). Full runbook: the Plan 6 doc above. Rollback stays GTM version 13 (`docs/GTM-NPHLG9NR_v13.json`).
