...
**N-1 `page_location` redaction — code landed, container publish PENDING.** `ga4BaseHtml()` and `ga4BridgeHtml()` in `scripts/gtm-api.mjs` now seed a redacted `page_location` into the GA4 config (and refresh it on SPA `page_view`), so GA4 auto-events (`session_start`/`first_visit`) that fire before the app can `history.replaceState` never carry `?order=`/`?payment_intent[_client_secret]=`/`?sale=`/`?preview=`. See `docs/superpowers/plans/2026-07-28-analytics-privacy-token-redaction.md` Task 4. **The container has not been republished yet** — until someone runs `npm run gtm:setup -- --publish` (needs `.secrets/gtm-api-deploy.json` + `GTM_*`/`NEXT_PUBLIC_*` env), the live container is still version 13 with the un-redacted base tag, and the app-layer strip (layer 1, shipped) is the only defence. On publishing, export the new version as `docs/GTM-NPHLG9NR_v14.json`, update this pointer, and **keep** `_v13.json` — it is the committed rollback reference for the Plan 6 native-tags migration (`docs/superpowers/plans/2026-07-28-gtm-native-tags-migration.md`), so v13 and v14 coexist until Plan 6 lands.
