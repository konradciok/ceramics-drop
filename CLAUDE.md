...
- **(Claude Code desktop only)** Persistent project memory for this repo lives under `~/.claude/projects/E--repositories-ceramics-drop/memory/` (on this machine `C:\Users\Komp\...`), indexed by `MEMORY.md`, loaded automatically; it reflects what was true when written — verify any file/flag it names still exists before acting. Cloud/remote agents (Cursor, CI) won't have this path — rely on `AGENTS.md` and `docs/` instead.
