...
- `.claude/settings.json` backs these rules with hooks: a PreToolUse guard blocks any `next build` lacking `--webpack`, a PostToolUse hook lints written `.ts`/`.tsx` files, and native worktrees symlink `node_modules` + `.next` from the main checkout (a fresh worktree needs no install).
